...
class CombinedPaa_Mgtuts:
    ...
    def parse(self, *, html) -> str:
        ...
        if _data:
            ...
            h2_tags = _data.find_all("h2")
            for count, h2_tag in enumerate(h2_tags):
                features[f"h2_{count+1}"] = h2_tag.text

                content: str = ''
                videos: List = []
                for sibs in h2_tag.find_next_siblings():
                    if sibs.name == 'div':
                        if sibs.has_attr("style"):
                            _videos = sibs.find_all("div",{"class":"rll-youtube-player"})
                            for _video in _videos:
                                videos.append(_video["data-src"])
        
                        # if sibs.has_attr("class"):
                        #     # print(sibs["class"])
                        #     if sibs["class"][0].startswith("code"):
                        #         continue
                        #     else:
                        #         content += sibs.text.strip() + '\n'
                        # else:
                        #     print(sibs)
                        #     content += sibs.text.strip() + '\n'
                ...
                features[f"content_{count+1}"] = content
                videos = list(set(videos))
                if videos:
                    features[f"videos_{count+1}"] = "\n".join(videos)
                    print(videos)
    ...
    def run(self):
        ...
        if (path.isdir("inputs")):
            ...
            for count, dat in enumerate(_final_urls):
                ...
                mgtuts_res = self.sub_mgtuts(url = url)
...
if __name__ == '__main__':
    ...
    #Run scraper
    scraper = CombinedPaa_Mgtuts(max_paa = max_paa, max_divisions = max_divisions)
    scraper.run()