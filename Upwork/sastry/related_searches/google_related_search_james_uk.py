...
class Google_Related_Search:
    def __init__(self, *,keywords: List = [], max_searches: int = 3, proxy_list:List = []) -> None:
        ...
        self.GEONODE_DNS: str = ''
        self.proxy_counter: int = 0
    ...
    def take_random_proxy(self):
        self.proxy_counter+=1
        try:
            self.GEONODE_DNS = self.proxy_list[self.proxy_counter]
        except IndexError:
            self.GEONODE_DNS = self.proxy_list[0]
    ...
    def fetch(self):
        ...
        while True:
            ...
            urlToGet = f"http://www.google.co.uk/search?q='{self.keyword}"
            # urlToGet = f"http://www.google.com/search?q='{self.keyword}"
            proxy = {"http":"http://{}:{}@{}".format(username, password, self.GEONODE_DNS)}
            response = requests.get(urlToGet , proxies=proxy)       

            print(self.keyword, " ", response.status_code, " ", self.GEONODE_DNS) 

            if response.status_code != 200:
                self.take_random_proxy()
                continue
            else:
                break
    ...
    def run(self):
        ...
        df = pd.read_csv("Products for Keywords - Rex - UK.csv", encoding='utf-8')
        df = df.set_index(["No"])
        ...
        try:
            ...
            for dat in df.index:   
                if not pd.isna(df.loc[dat,cols[0]]):
                    ...
                    print(f"Scraped {added_counter} root keyword: ", keyword)
                ...
                df.to_csv("Products for Keywords - Rex - UK.csv", encoding = "utf-8")
                print("Saved.")
...
if __name__ == '__main__':
    ...
    proxies = open('Residential Premium-Rotating Proxy.txt', 'r', encoding='utf-8')
    proxy_list: List = [acc.strip() for acc in proxies.readlines()]

    
    #Run scraper
    scraper = Google_Related_Search(keywords= keywords, max_searches = max_searches, proxy_list = proxy_list)
    scraper.run()
