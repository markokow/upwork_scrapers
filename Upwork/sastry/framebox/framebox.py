...
class CombinedPaa_FrameBox:
    ...
    def parse(self, *, html) -> str:
        ...
        if _data:
            ...
            _table_of_contents = _data.find("div", {"class":"table-of-contents"})
            _eztoc = _data.find("div", {"id":"ez-toc-container"})
            _code_blocks = _data.find_all("div", {"class":"code-block"})
            _youtube_ads = _data.find_all("div", {"clas s":"rll-youtube-player"})
            _imgs = _data.find_all("img")
            # _noscript = _data.find_all("noscript")

            if _table_of_contents:
                _content = str(_data).replace(str(_table_of_contents), "")
            else:
                _content = str(_data)

            if _eztoc:
                _content = str(_content).replace(str(_eztoc), "")
            else:
            ...
            if _imgs:
                for _img in _imgs:
                    _content = str(_content).replace(str(_img), "")

            # if _noscript:
            #     for _ in _noscript:
            #         _content = str(_content).replace(str(_), "")
            
            _content = _content.replace("<p></p>","")
            _content = _content.replace("<noscript></noscript>","")
            
            _content = _content.replace("<div class=\"entry-content\" itemprop=\"articleBody\">", "")

            idx = self.find_str(s = _content, char = '<!-- AI CONTENT END 3 -->')
            _content = _content[:idx]
        ...
        if (path.isdir("inputs")):
            ...
            for count, dat in enumerate(_final_urls):
                ...
                while True:
                    time.sleep(3)
                    try:    
                        framebox_res = self.sub_framebox(url = url)
                        break
                    except requests.ConnectTimeout:
                        print("connection have timed out, resetting in 3 seconds")
                        continue
                ...
                if (count == (len(_final_urls) - 1)) | (file_counter == _size):
                    ...
                    _filename = f"{loc.replace('.txt', '').replace('inputs', 'outputs')}_part_{part}.csv"
...
if __name__ == '__main__':
    ...
    #Run scraper
    scraper = CombinedPaa_FrameBox(max_paa = max_paa, max_divisions = max_divisions)
    scraper.run()